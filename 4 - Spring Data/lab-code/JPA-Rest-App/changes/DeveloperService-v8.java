    ...
    if (allParams.size() == 0)
      ...
      for (Map.Entry<String, String> entry : allParams.entrySet()) {
        ...
        if (entry.getKey().equals(LANGUAGE_PARAM)) {
          langParam = entry.getValue();
        }
      
      }
      
      if (langParam != null) {
        log.info("Retrieving all developers who can code in : " +langParam);
        listToReturn = convertDeveloperListToDeveloperDTOList(devRepo.findByLanguagesContaining(langParam));
      } 
